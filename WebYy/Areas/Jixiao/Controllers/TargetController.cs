...
namespace WebProject.Areas.Jixiao.Controllers
{
    public class TargetController : BaseLoginController
    {
        ...
        {
            var p_jixiao_target = DoMySql.FindField<ModelDb.p_jixiao_target>("sum(amount),sum(new_num)", $"tg_user_sn in {new DomainUserBasic.UserRelationApp().GetNextUserSnForSql(ModelEnum.UserRelationTypeEnum.运营邀厅管, new UserIdentityBag().user_sn)} and yearmonth = '{DateTime.Today.ToString("yyyy-MM")}'");
        ...
        $"from p_jixiao_day " +
        $"where tg_user_sn in {new DomainUserBasic.UserRelationApp().GetNextUserSnForSql(ModelEnum.UserRelationTypeEnum.运营邀厅管, new UserIdentityBag().user_sn)} and c_date>='{DateTime.Today.ToString("yyyy-MM-01")}' and c_date<'{DateTime.Today.AddMonths(1).ToString("yyyy-MM-01")}' " +
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult TGDayTarget(string date, string tg_user_sn, string type = "amount")
        {
