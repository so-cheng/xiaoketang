...
using static Services.Project.PageFactory;
...
namespace WebProject.Areas.Jixiao.Controllers
{
    public class DangBiaoController : BaseLoginController
    {
        ...
        {
            var req = new PageFactory.TableList.DtoReq();
            req.relation_type = ModelEnum.UserRelationTypeEnum.运营邀厅管;
            var pageModel = new PageFactory.TableList().Get(req);
        ...
        public ActionResult ShowCurrentTable(string tg_user_sn)
        {
            ...
            //1.根据当前的时间查找档表
            var p_jixiao_dangbiao = DomainBasicStatic.DoMySql.FindEntity<ModelDb.p_jixiao_dangbiao>($"tg_user_sn = '{tg_user_sn}' order by e_date desc", false);
            if (p_jixiao_dangbiao.IsNullOrEmpty())
            {
                p_jixiao_dangbiao = new ModelDb.p_jixiao_dangbiao();
            }
            ViewBag.isOverTime = false;
            if (p_jixiao_dangbiao.e_date < DateTime.Today)
            {
                ViewBag.isOverTime = true;
            }
            var tableItem = DomainBasicStatic.DoMySql.FindEntity<ModelDb.p_jixiao_dangbiao_item>($"db_sn = '{p_jixiao_dangbiao.db_sn}'", false);
