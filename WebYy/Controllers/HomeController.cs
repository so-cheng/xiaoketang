...
namespace WebProject.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        {
            var req = new PageFactory.UserInfo.Yy_AccountEdit.DtoReq();
            req.id = new UserIdentityBag().id;
            var pageModel = new PageFactory.UserInfo.Yy_AccountEdit().Get(req);
        ...
        {
            var notice = DoMySql.FindEntityById<ModelDb.sys_notice>(notice_id);
            if (notice.IsNullOrEmpty())
            {
                return Content("消息不存在");
            }
            if (notice.user_sn != new UserIdentityBag().user_sn)
            {
                return Content("当前用户无权查看");
            }
            notice.is_read = ModelDb.sys_notice.is_read_enum.已读.ToSByte();
            notice.Update();
            return RedirectToAction(notice.link_url);
        }
