...
namespace WebProject.Areas.Work.Controllers
{
    public class TodoRuleController : BaseLoginController
    {
        public ActionResult List(int status = 0)
        {
            ...
            pageModel.listDisplay.listItems.Where(x => x.field == "zt_sn_test").FirstOrDefault().disabled = true;
            pageModel.listFilter.formItems.Where(x => x.name == "zt_sn").FirstOrDefault().disabled = true;
        ...
        public ActionResult Post(int id = 0)
        {
            ...
            pageModel.formDisplay.formItems.Where(x => x.name == "zt_sn").FirstOrDefault().displayStatus = EmtModelBase.DisplayStatus.隐藏;
            return View(pageModel);
        }    
    }
}