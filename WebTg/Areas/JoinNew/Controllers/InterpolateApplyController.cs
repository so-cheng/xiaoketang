...
namespace WebProject.Areas.JoinNew.Controllers
{
    public class InterpolateApplyController : BaseLoginController
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ActionResult InterpolateApply(string ting_sn, int id = 0)
        {
            var req = new PageFactory.JoinNewPush.NtPost.DtoReq();
            req.ting_sn = ting_sn;
            req.id = id;
            var pageModel = new PageFactory.JoinNewPush.NtPost().Get(req);
            return View(pageModel);
        }
        #endregion
    }
