...
namespace WebProject.Areas.Jixiao.Controllers
{
    public class TargetController : BaseLoginController
    {
        ...
        {
            var p_jixiao_target = DoMySql.FindField<ModelDb.p_jixiao_target>("sum(amount),sum(new_num)", $"tg_user_sn ='{new UserIdentityBag().user_sn}' and yearmonth = '{DateTime.Today.ToString("yyyy-MM")}'");

            ViewBag.amount = p_jixiao_target[0].IsNullOrEmpty() ? 0 : p_jixiao_target[0].ToInt();
            ViewBag.new_num = p_jixiao_target[1].IsNullOrEmpty() ? 0 : p_jixiao_target[1].ToInt();

            var jixao_month = DoMySql.FindField<ModelDb.p_jixiao_day>("sum(amount),sum(new_num),sum(contact_num),sum(datou_num)", $"tg_user_sn ='{new UserIdentityBag().user_sn}' and c_date >= '{DateTime.Today.ToString("yyyy-MM-01")}' and  c_date < '{DateTime.Today.AddMonths(1).ToString("yyyy-MM-01")}'");

            ViewBag.month_amount = jixao_month[0].IsNullOrEmpty() ? 0 : jixao_month[0].ToInt();
            ViewBag.month_new_num = jixao_month[1].IsNullOrEmpty() ? 0 : jixao_month[1].ToInt();
        ...
        $"from p_jixiao_day " +
        $"where tg_user_sn ='{new UserIdentityBag().user_sn}' and c_date>='{DateTime.Today.ToString("yyyy-MM-01")}' and c_date<'{DateTime.Today.AddMonths(1).ToString("yyyy-MM-01")}' " +
        $"group by c_date order by c_date");
            ...
            foreach (var item in list)
            {
                ...
                amountArray[i] = item.sum_amount.ToNullableString();
                newNumArray[i] = item.sum_new_num.ToNullableString();
                contactNumArray[i] = item.sum_contact_num.ToNullableString();
                datouNumArray[i] = item.sum_datou_num.ToNullableString();
                i++;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DayTarget(string date, string zb_user_sn, string type = "amount")
        {
