...
$(function () {
    ...
    //30秒刷新1次数据
    setInterval(function () { test() }, 30 * 1000);
    ...
    function test() {
        ...
        $.ajax(
            {
                type: "POST",
                url: "/Corp/GetLargeData",
                success: function (result) {
                    var left = month_left();
                    //汇总
                    $("#plan_amout_t").html(Thousandformat(result.Data.plan_amout_t));
                    $("#real_amout_t").html(Thousandformat(result.Data.real_amout_t));
                    if (last_real != result.Data.real_amount_t) {
                        $('#real_amount_d').lemCounter({
                            value_to: result.Data.real_amount_d,
                            value_from: last_real
                        });
                        last_real = result.Data.real_amount_t;
                    }
    ...
    //红包
    function showImg(photo, who, money) {
        $("#photo").attr('src', "http://b.crm.91jingyun.com" + photo);
        $("#who").html(who);
        $("#money").html(money);
        $("#container").show();
        var mp3 = "ding" + random(1, 13) + ".mp3";
        $('#myaudio').attr('src', 'http://b.crm.91jingyun.com/Files/' + mp3);
        var myAuto = document.getElementById('myaudio');

        var limit_t = new Date().getHours();
        if (limit_t > 13) {
            myAuto.play();
            setTimeout(function () { myAuto.pause(); myAuto.load(); }, 15000);
        }
        if (limit_t < 12) {
            myAuto.play();
            setTimeout(function () { myAuto.pause(); myAuto.load(); }, 15000);
