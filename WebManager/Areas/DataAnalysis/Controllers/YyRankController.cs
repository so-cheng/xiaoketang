...
namespace WebProject.Areas.DataAnalysis.Controllers
{
    public class YyRankController : BaseLoginController
    {
        ...
        /// <param name="yy_user_sn">运营sn（传值为空时显示所有数据）</param>
        ...
        public ActionResult LineChart(string c_date, string yy_user_sn, string type)
        {
            ...
            //获取时间段内每天的运营绩效数据
            var where = $"tenant_id = '{new DomainBasic.TenantApp().GetInfo().id}' and DATE(c_date) between '{f_date}' and '{t_date}' and (new_num!=0 or amount_1!=0 or num_2!=0 or amount_2!=0 or contact_num!=0)";
            if (!yy_user_sn.IsNullOrEmpty())
            {
                where += $" and yy_user_sn = '{yy_user_sn}'";
            }
            var jixiaoDayList = DoMySql.FindList<ModelDb.p_jixiao_day>(where);
