...
function win_pop_layer(title, module_id, area_width, area_height, win_layer, data_url) {
    var _area_width = "950";
    var _area_height = "520";
    if (area_width !== undefined) {
        _area_width = area_width;
...
function reStructureObject(obj, names, o) {
    for (let key in obj) {
        ...
        if (key.indexOf("l_") === 0) {
            var tmp = [];
            var obj_key = obj[key];
            var _item_len = obj_key[Object.keys(obj_key)[0]];
            if (typeof (_item_len) === "object") {
                for (var len = 0; len < _item_len.length; len++) {
                    var _obj = {};
        ...
        if (key.indexOf("j_") === 0) {
            console.log(names);
            //eval("o" + names + " = " + JSON.stringify(obj_key));
        }
