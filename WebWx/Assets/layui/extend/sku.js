...
; layui.define(['jquery', 'form'], function (e) {
    "use strict"; var $ = layui.jquery, form = layui.form, o = {
        ...
        y: function (s, d, a, h) {
            ...
            {
                h += '<input type="checkbox" data-id="' + s + '_' + d[i].id + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="item"' + this.z(s, d[i].id, a) + '>';
            }
            h += "<a style='line-height:36px;margin-left:8px;'>添加规格值</a>";
            return h;
        },
        x: function (d, a, h) {
        ...
        st: function () {
            return '<table class="layui-table" lay-skin=""><thead>' + this.sth() + '</thead><tbody>' + this.stb() + this.stf() + '</tbody></table>';
        },
        sth: function () {
            var h = '<tr>'; for (var i = 0; i < this.c.tableTitle.length; i++) { h += '<th style="text-align:center;">' + this.c.tableTitle[i] + '</th>'; } h += '<th style="text-align:center;">成本价</th><th style="text-align:center;">库存</th><th style="text-align:center;">编号</th></tr>'; return h;
        },
        stb: function () {
            var t = this, h = '', na = [];
            for (var i = 0; i < t.c.data.length; i++) {
                ...
                h += '<td style="text-align:center;"><input class="layui-input layui-table-input" data-index="' + i + '" data-type="price" lay-filter="input"  value="' + t.c.data[i].price + '" ></td>';
                h += '<td style="text-align:center;"><input class="layui-input layui-table-input" data-index="' + i + '" data-type="stock" lay-filter="input" value="' + t.c.data[i].stock + '" ></td>';
                h += '<td style="text-align:center;"><input class="layui-input layui-table-input" data-index="' + i + '" data-type="sku" lay-filter="input" value="' + t.c.data[i].sku + '" ></td>';
                h += '</tr>';
            }
            return h;
        },
