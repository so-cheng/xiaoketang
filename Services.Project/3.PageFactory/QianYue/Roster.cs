    ...
    public partial class PageFactory
    {
        ...
        public class List
        {
            ...
            public ModelBasic.CtlListFilter GetListFilter(DtoReq req)
            {
                ...
                listFilter.formItems.Add(new ModelBasic.EmtInput("jjr_name")
                {
                    width = "120px",
                    placeholder = "经纪人",
                });
            ...
            public ModelBasic.CtlListDisplay GetListDisplay(DtoReq req = null)
            {
                ...
                listDisplay.listItems.Add(new ModelBasic.EmtModel.ListItem("qun")
                {
                    text = "培训群",
                    width = "100",
                    minWidth = "100"
                });
                listDisplay.listItems.Add(new ModelBasic.EmtModel.ListItem("is_qun_name")
                {
                    text = "是否进群",
                    width = "100",
                    minWidth = "100"
            ...
            public ModelResult.List GetListData(ModelBasic.CtlListDisplay.ListData.Req reqJson)
            {
                ...
                {
                    where += $" and dou_account = '{req["dou_account"]}'";
                }
                if (!req["wx_account"].ToNullableString().IsNullOrEmpty())
                {
                    where += $" and wx_account = '%{req["wx_account"]}%'";
                }
                if (!req["jjr_name"].ToNullableString().IsNullOrEmpty())
                {
                    where += $" and jjr_name = '{req["jjr_name"]}'";
                }
                if (!req["real_name"].ToNullableString().IsNullOrEmpty())
                {
                    where += $" and real_name = '{req["real_name"]}'";
                }
        ...
        public class Post
        {
            ...
            private ModelBasic.CtlFormDisplay GetFormDisplay(ModelBasic.PagePost pageModel, DtoReq req)
            {
                ...
                formDisplay.formItems.Add(new ModelBasic.EmtSelect("sex")
                {
                    ...
                    defaultValue = p_qianyue_roster.sex.ToString()
                });
                ...
                formDisplay.formItems.Add(new ModelBasic.EmtSelect("is_hudong")
                {
                    title = "是否互动",
                    colLength = 6,
                    options = new Dictionary<string, string>()
                    ...
                    defaultValue = p_qianyue_roster.is_hudong > 0 ? p_qianyue_roster.is_hudong.ToInt().ToNullableString() : "1",
                });
                ...
                formDisplay.formItems.Add(new ModelBasic.EmtInput("qun")
                {
                    title = "培训群",
                    colLength = 6,
                    defaultValue = p_qianyue_roster.qun
                });
                formDisplay.formItems.Add(new ModelBasic.EmtSelect("is_qun")
                {
                    title = "是否进群",
                    colLength = 6,
                    options = new Dictionary<string, string>()
                    ...
                    defaultValue = p_qianyue_roster.is_qun > 0 ? p_qianyue_roster.is_qun.ToInt().ToNullableString() : "1",
