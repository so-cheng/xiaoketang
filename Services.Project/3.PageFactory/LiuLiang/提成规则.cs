    ...
    public partial class PageFactory
    {
        public partial class LiuLiang
        {
            ...
            public class CommissionRulePost
            {
                ...
                {
                    var p_liuliang_commission_rule = DomainBasicStatic.DoMySql.FindEntity<ModelDb.p_liuliang_commission_rule>($"id = {req.id}", false);
                    var formDisplay = pageModel.formDisplay;
                    #region 表单元素
                    formDisplay.formItems.Add(new EmtHidden("id")
                    {
                        defaultValue = p_liuliang_commission_rule.id.ToString()
                    });

                    formDisplay.formItems.Add(new EmtInput("num_s")
                    {
                        title = "推出人数起始",
                        colLength = 6,
                        defaultValue = p_liuliang_commission_rule.num_s.ToString()
                    });

                    formDisplay.formItems.Add(new EmtInput("num_e")
                    {
                        title = "推出人数结束",
                        colLength = 6,
                        defaultValue = p_liuliang_commission_rule.num_e.ToString()
                    });
                    ...
                    formDisplay.formItems.Add(new EmtInputMoney("amount")
                    {
                        title = "提成",
                        colLength = 6,
                        defaultValue = p_liuliang_commission_rule.amount.ToString()
                    });
                    #endregion
                    return formDisplay;
                }
                ...
                public JsonResultAction PostAction(JsonRequestAction req)
                {
                    ...
                    var p_liuliang_commission_rule = req.data_json.ToModel<ModelDb.p_liuliang_commission_rule>();
                    if (p_liuliang_commission_rule.amount.IsNullOrEmpty()) throw new WeicodeException("请输入提成");

                    p_liuliang_commission_rule.tenant_id = new DomainBasic.TenantApp().GetInfo().id;
                    p_liuliang_commission_rule.InsertOrUpdate();
            ...
            public class CommissionRuleList
            {
                ...
                private ModelBasic.CtlListDisplay GetListDisplay(DtoReq req = null)
                {
                    ...
                    listDisplay.listItems.Add(new ModelBasic.EmtModel.ListItem("num_s")
                    {
                        text = "推出人数起始",
                        width = "120",
                        minWidth = "120"
                    });
                    listDisplay.listItems.Add(new ModelBasic.EmtModel.ListItem("num_e")
                    {
                        text = "推出人数结束",
                        width = "120",
                        minWidth = "120"
                ...
                public ModelResult.List GetListData(ModelBasic.CtlListDisplay.ListData.Req reqJson)
                {
                    ...
                    {
                        where = where,
                        orderby = "order by num_s"
                    };
                    return new ModelBasic.CtlListDisplay.ListData().getList<ModelDb.p_liuliang_commission_rule, ItemDataModel>(filter, reqJson);
                }
                ...
                /// 数据项模型
                /// </summary>
                public class ItemDataModel : ModelDb.p_liuliang_commission_rule
                {
                ...
                public JsonResultAction DelAction(JsonRequestAction req)
                {
                    ...
                    var reqData = req.data_json.ToModel<ModelDb.p_liuliang_commission_rule>();
                    var p_liuliang_commission_rule = new ModelDb.p_liuliang_commission_rule();

                    lSql.Add(p_liuliang_commission_rule.DeleteTran($"id in ({reqData.id})"));
