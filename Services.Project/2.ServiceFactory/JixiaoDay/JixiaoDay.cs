    ...
    public partial class ServiceFactory
    {
        public partial class JixiaoDay
        {
            public class JixiaoService
            {
                ...
                /// <returns></returns>
                public ModelDb.p_jixiao_day GetZbJixiaoOnDate(string zb_user_sn, DateTime c_date)
                {
                    var p_jixiao_day = DoMySql.FindEntity<ModelDb.p_jixiao_day>($" zb_user_sn = '{zb_user_sn}' and c_date = '{c_date.ToString("yyyy-MM-dd")}'", false);
                    return p_jixiao_day;
                }
                ...
                /// <param name="c_date"></param>
                /// <returns></returns>
                public int GetZbReportNumByTingSn(string ting_sn, DateTime c_date)
                {
                    var list = DoMySql.FindList<ModelDb.p_jixiao_day_session>($"ting_sn='{ting_sn}' and c_date='{c_date}' GROUP BY zb_user_sn");
                    return list.Count;
                }
                ...
                /// <param name="c_date"></param>
                /// <returns></returns>
                public int GetDangNumByTingSn(string ting_sn, DateTime c_date)
                {
                    var list = DoMySql.FindField<ModelDb.p_jixiao_day_session>("COUNT(DISTINCT session)", $"c_date='{c_date}' and ting_sn='{ting_sn}'");
                    return list[0].ToInt();
                }
                ...
                /// <param name="c_date"></param>
                /// <returns></returns>
                public double GetAmount2AVG(string ting_sn,DateTime c_date)
                {
                    var list = DoMySql.FindList<ModelDb.p_jixiao_day>($"ting_sn = '{ting_sn}' and c_date = '{c_date}'");
                    if (list.Count==0) { return 0; }
                    return list.Sum(x => x.amount_2).ToDouble() / list.Count;
                }
            }
            public class JixiaoTable : ModelDb.p_jixiao_day
            {
                ...
                public object hx_amount_sum { get; set; }
                public object hx_num_sum { get; set; }
                public object hdpk_amount_sum { get; set; }
            }
        }
    }
}
